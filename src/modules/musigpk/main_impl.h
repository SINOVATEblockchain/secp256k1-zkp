...
static int secp256k1_musigpk_compute_ell(const secp256k1_context *ctx, unsigned char *ell, const secp256k1_pubkey *pk, size_t np) {
...
static void secp256k1_musigpk_sha256_init_tagged(secp256k1_sha256_t *sha) {
    secp256k1_sha256_initialize(sha);
...
static void secp256k1_musigpk_coefficient(secp256k1_scalar *r, const unsigned char *ell, uint32_t idx) {
    secp256k1_sha256_t sha;
    unsigned char buf[32];
    size_t i;

    secp256k1_musigpk_sha256_init_tagged(&sha);
    secp256k1_sha256_write(&sha, ell, 32);
    /* We're hashing the index of the signer instead of its public key as specified
     * in the musigpk paper. This reduces the total amount of data that needs to be
     * hashed.
     * Additionally, it prevents creating identical musigpk_coefficients for identical
     ...
     * a valid partial signature. This is not a problem for musigpk per se, but could
...
} secp256k1_musigpk_pubkey_combine_ecmult_data;

/* Callback for batch EC multiplication to compute ell_0*P0 + ell_1*P1 + ...  */
static int secp256k1_musigpk_pubkey_combine_callback(secp256k1_scalar *sc, secp256k1_ge *pt, size_t idx, void *data) {
    secp256k1_musigpk_pubkey_combine_ecmult_data *ctx = (secp256k1_musigpk_pubkey_combine_ecmult_data *) data;
    secp256k1_musigpk_coefficient(sc, ctx->ell, idx);
    return secp256k1_pubkey_load(ctx->ctx, pt, &ctx->pks[idx]);
}


static void secp256k1_musigpk_signers_init(secp256k1_musigpk_session_signer_data *signers, uint32_t n_signers) {
    uint32_t i;
...
int secp256k1_musigpk_pubkey_combine(const secp256k1_context* ctx, secp256k1_scratch_space *scratch, secp256k1_pubkey *combined_pk, unsigned char *pk_hash32, const secp256k1_pubkey *pubkeys, size_t n_pubkeys) {
    secp256k1_musigpk_pubkey_combine_ecmult_data ecmult_data;
    secp256k1_gej pkj;
    secp256k1_ge pkp;
    ...
    if (!secp256k1_musigpk_compute_ell(ctx, ecmult_data.ell, pubkeys, n_pubkeys)) {
        return 0;
    }

    if (!secp256k1_ecmult_multi_var(&ctx->error_callback, &ctx->ecmult_ctx, scratch, &pkj, NULL, secp256k1_musigpk_pubkey_combine_callback, (void *) &ecmult_data, n_pubkeys)) {
        return 0;
    }
...
int secp256k1_musigpk_session_initialize(const secp256k1_context* ctx, secp256k1_musigpk_session *session, secp256k1_musigpk_session_signer_data *signers, unsigned char *nonce_commitment32, const unsigned char *session_id32, const unsigned char *msg32, const secp256k1_pubkey *combined_pk, const unsigned char *pk_hash32, size_t n_signers, size_t my_index, const unsigned char *seckey) {
    ...
    secp256k1_musigpk_signers_init(signers, session->n_signers);
    ...
    secp256k1_musigpk_coefficient(&mu, pk_hash32, (uint32_t) my_index);
...
int secp256k1_musigpk_session_get_public_nonce(const secp256k1_context* ctx, secp256k1_musigpk_session *session, secp256k1_musigpk_session_signer_data *signers, secp256k1_pubkey *nonce, const unsigned char *const *commitments, size_t n_commitments, const unsigned char *msg32) {
    secp256k1_sha256_t sha;
...
int secp256k1_musigpk_session_initialize_verifier(const secp256k1_context* ctx, secp256k1_musigpk_session *session, secp256k1_musigpk_session_signer_data *signers, const unsigned char *msg32, const secp256k1_pubkey *combined_pk, const unsigned char *pk_hash32, const unsigned char *const *commitments, size_t n_signers) {
    size_t i;
...
int secp256k1_musigpk_set_nonce(const secp256k1_context* ctx, secp256k1_musigpk_session_signer_data *signer, const secp256k1_pubkey *nonce) {
    unsigned char commit[33];
    size_t commit_size = sizeof(commit);
    secp256k1_sha256_t sha;
...
int secp256k1_musigpk_session_combine_nonces(const secp256k1_context* ctx, secp256k1_musigpk_session *session, const secp256k1_musigpk_session_signer_data *signers, size_t n_signers, int *nonce_is_negated, const secp256k1_pubkey *adaptor) {
     ...
     * been set in `musigpk_session_get_public_nonce`. */
...
int secp256k1_musigpk_partial_signature_serialize(const secp256k1_context* ctx, unsigned char *out32, const secp256k1_musigpk_partial_signature* sig) {
    VERIFY_CHECK(ctx != NULL);
...
int secp256k1_musigpk_partial_signature_parse(const secp256k1_context* ctx, secp256k1_musigpk_partial_signature* sig, const unsigned char *in32) {
    VERIFY_CHECK(ctx != NULL);
    ARG_CHECK(sig != NULL);
    ARG_CHECK(in32 != NULL);
...
static int secp256k1_musigpk_compute_messagehash(const secp256k1_context *ctx, unsigned char *msghash, const secp256k1_musigpk_session *session) {
...
int secp256k1_musigpk_partial_sign(const secp256k1_context* ctx, const secp256k1_musigpk_session *session, secp256k1_musigpk_partial_signature *partial_sig) {
    ...
    /* build message hash */
    if (!secp256k1_musigpk_compute_messagehash(ctx, msghash, session)) {
        return 0;
    }
...
int secp256k1_musigpk_partial_sig_combine(const secp256k1_context* ctx, const secp256k1_musigpk_session *session, secp256k1_schnorr *sig, const secp256k1_musigpk_partial_signature *partial_sigs, size_t n_sigs, const unsigned char *tweak32) {
    size_t i;
    secp256k1_scalar s;
    secp256k1_ge noncep;
    (void) ctx;
...
int secp256k1_musigpk_partial_sig_verify(const secp256k1_context* ctx, const secp256k1_musigpk_session *session, const secp256k1_musigpk_session_signer_data *signer, const secp256k1_musigpk_partial_signature *partial_sig, const secp256k1_pubkey *pubkey) {
    unsigned char msghash[32];
    secp256k1_scalar s;
    secp256k1_scalar e;
    secp256k1_scalar mu;
    ...
    /* Multiplying the messagehash by the musigpk coefficient is equivalent
     * to multiplying the signer's public key by the coefficient, except
     * much easier to do. */
    secp256k1_musigpk_coefficient(&mu, session->pk_hash, signer->index);
...
int secp256k1_musigpk_partial_sig_adapt(const secp256k1_context* ctx, secp256k1_musigpk_partial_signature *adaptor_sig, const secp256k1_musigpk_partial_signature *partial_sig, const unsigned char *sec_adaptor32, int nonce_is_negated) {
    secp256k1_scalar s;
    secp256k1_scalar t;
    int overflow;
...
int secp256k1_musigpk_extract_secret_adaptor(const secp256k1_context* ctx, unsigned char *sec_adaptor32, const secp256k1_schnorr *sig, const secp256k1_musigpk_partial_signature *partial_sigs, size_t n_partial_sigs, int nonce_is_negated) {
    secp256k1_scalar t;
    secp256k1_scalar s;
    int overflow;
    size_t i;
